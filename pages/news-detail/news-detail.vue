<template>
	<view class="news_detail">
		...
		<view class="info">
			<text>发表时间：{{detail.year | formatDate}}</text>
			<text>浏览人数：{{detail.numberViews}}</text>
		</view>
		<view class="content">
			<rich-text :nodes="contents"></rich-text>
			<image :src="detail.imgUrl" style="width: 100%; height: 800rpx;"></image>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			async getNewDetail(){
				const res = await this.$myRequest({
					url:'newsInfo/getOneById?id='+this.id
				});
		...
		onLoad(options) {
			// 'options' type is object ,属于上一级页面带有的参数
			// console.log(options)
			this.id=options.id
			this.getNewDetail()
		}
